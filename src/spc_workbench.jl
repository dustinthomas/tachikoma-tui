...
function view(m::SPCWorkbenchModel, f::Frame)
    ...
    # header
    hdr = "SPC Workbench [dashboard]  [m]lib [p]pause [r]reset [c]config [u/t/l/s]specs [1-8]rules [h]help [k]keys [[]]chart [q]quit"
    ...
    render(StatusBar(left=[Span(left, tstyle(:text_dim))], right=[Span("[m]lib [p r c v o u t l s] [h k []] [q]", tstyle(:text_dim))]), footer, buf)
end
